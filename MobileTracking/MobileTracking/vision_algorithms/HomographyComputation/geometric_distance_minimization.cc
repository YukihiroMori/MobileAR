//
// Geometric distance minimization for homography computation
//
...
bool
HomographyComputation::geometric_distance_minimization(
   Vector2d pos0[],   // Data:                      INPUT
   Vector2d pos1[],   // Data:                      INPUT
   int      Num,      // Number of Data:            INPUT
   Vector9d& theta0,   // Homography:                OUTPUT
...
)
{
   ...
   do
   {
	 ...
	 for (int k = 0; k < 3; k++)
	    for (int l = 0; l < 3; l++)
	       M += Wa[i](k,l)*xast[i][k]*xast[i][l].transpose();
      }
      M /= (double)Num;
...
)
{
   ...
   flag = geometric_distance_minimization(pos0, pos1, Num, theta,
                                          Max_Iter, Conv_EPS, f0);

   // update when the correction successed
   if (flag)  H = convert_mat3(theta);
